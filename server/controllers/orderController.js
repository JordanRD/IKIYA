...
module.exports = {
    checkout: async (req, res) => {
        ...
        try {
            const checkStockQuery = 'SELECT od.id_order,od.id_product,od.qty,total_stock,id_product_status FROM order_details od JOIN (SELECT  id_product, SUM(stock - purchased_stock) total_stock FROM storages GROUP BY id_product) s ON s.id_product = od.id_product join products p on p.id_product=od.id_product WHERE od.id_order = ? having id_product_status!=1 or total_stock-qty<0;'
            const [checkStock]=await asyncQuery(checkStockQuery,[id_order])
            if (checkStock) return res.status(400).send('Some product on your cart is not available please remove it or change the qty')
            // return res.status(200).send('mnnnn')
            const query = [
        ...
        try {
            ...
            for await (let cart of userCart) {
                ...
                for await (let store of sorted) {
                    ...
                    if (currentQty > 0 && selectedStorage) {
                        await asyncQuery(query[4], [Math.max(selectedStorage.stock - currentQty, 0), cart.id_product, store.id_store])
